...
public class ArchonRobot extends SmartBaseRobot {
	...
	private static final int FAST_PRODUCE_FARMER_LIMIT = 9;
	private static final float BULLET_THRESHOLD_GARDENER_FAST_PRODUCE = 140f;
	private static final float BULLET_THRESHOLD_GARDENER_SLOW_PRODUCE = 1000f;
	...
	protected void onGameRound(RobotController rc) throws Exception {
		...
		MapLocation myLocation = getCachedLocation();
		...
		Integer gardenerNearbyCount = 0;
		if (!rc.hasMoved() && mDirection == null) {
            for (RobotInfo robot : visibleFriendlies) {
                ...
                if (robot.getType() == RobotType.GARDENER) {
					...
					gardenerNearbyCount++;
                    if (robot.location.distanceTo(myLocation) < RobotType.GARDENER.bodyRadius * 7) {
                        mDirection = new Direction(robot.location, myLocation);
                        rc.setIndicatorLine(myLocation, robot.location, 255, 0, 0);
                    }
                }
            }

			// try to build a farmer if necessary
			if (gardenerNearbyCount < 4 && shouldBuildFarmer(bulletCount)) {
				for (int i = 0; i < 8; i++ ) {
